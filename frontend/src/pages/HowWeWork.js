...
export default function HowWeWork() {
  ...
  return (
    <div ref={ref} className="pt-[68px]" data-testid="how-we-work-page">
      ...
      <section className="mx-auto max-w-[1280px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <Reveal delay={220} as="p" className="font-mono-sys mt-4 text-[11.5px] text-[#232A2A]/55">The brief can be messy. The thinking won’t be.</Reveal>
      </section>
              ...
              <article className={`grid gap-6 lg:grid-cols-12 ${i % 2 === 0 ? "" : "lg:text-right"}`} data-testid={`hww-stage-${s.label.toLowerCase()}`}>
                <div className={`lg:col-span-5 ${i % 2 === 0 ? "" : "lg:col-start-8"}`}>
                  <div className={`panel-dark p-7 sm:p-8 ${i % 2 === 0 ? "" : "lg:ml-auto"}`}>
                    ...
                    <h2 className="font-display mt-3 text-5xl leading-none text-[#F7F5EE] sm:text-6xl">{s.label}</h2>
                    <p className="mt-2 text-lg font-bold text-[#F19020]">{s.page}</p>
                    <p className="mt-1 text-[15px] font-semibold text-[#F7F5EE]/85">{s.title}</p>

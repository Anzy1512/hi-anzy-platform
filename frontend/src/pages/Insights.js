...
export default function Insights() {
  ...
  return (
    <div ref={ref} className="pt-[68px]" data-testid="insights-page">
      ...
      <section className="mx-auto max-w-[1280px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <Reveal delay={80}>
          <h1 className="font-display mt-5 leading-[0.88] text-[#232A2A] text-6xl sm:text-7xl lg:text-8xl" data-testid="insights-h1">
            Notes From the Work
          </h1>
        </Reveal>
        <Reveal delay={160} as="p" className="font-mono-sys mt-5 text-[11.5px] text-[#232A2A]/55">Useful first. Search engine second.</Reveal>
            ...
            {posts.map((p, i) => (
              <Reveal key={p.slug} delay={(i % 2) * 90}>
                <Link to={`/insights/${p.slug}`} className="case-card group block h-full rounded-[18px] border border-[#232A2A]/15 bg-[#F7F5EE] p-7" data-testid={`insight-card-${p.slug}`}>
                  ...
                  <h2 className="font-display mt-4 text-3xl leading-[0.95] text-[#232A2A] sm:text-4xl">{p.title}</h2>
                  <p className="mt-3 text-[14px] leading-relaxed text-[#232A2A]/78">{p.excerpt}</p>
